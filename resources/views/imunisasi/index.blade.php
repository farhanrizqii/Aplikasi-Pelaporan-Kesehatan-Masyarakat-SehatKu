            ...
            <div class="data-card">
                <div class="text-gray-900 dark:text-gray-100">
                    ...
                    <div class="action-buttons-container"> 
                        <a href="{{ route('imunisasi.create') }}" class="btn-tambah">
                            <i class="fas fa-plus"></i>
                            <span>Catat Imunisasi Baru</span>
                        </a>
                        <a href="{{ route('imunisasi.export') }}" class="btn-ekspor">
                    ...
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('imunisasi.index') }}" class="flex items-center w-full md:w-1/3 mb-8">
                        <input type="text" name="search" placeholder="Cari Nama atau Jenis Imunisasi..." 
                            value="{{ request('search') }}"
                            class="search-input"
                            aria-label="Cari data imunisasi">
                        ...
                        @if (request('search'))
                            <a href="{{ route('imunisasi.index') }}" class="text-sm text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 font-semibold ml-3">Reset</a>
                        @endif
                    </form>
                    ...
                    <div class="overflow-x-auto">
                            ...
                            <table class="data-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left">Nama Penduduk</th>
                                        <th class="text-left">Jenis Imunisasi</th>
                                ...
                                <tbody>
                                    @foreach ($imunisasis as $index => $imunisasi)
                                        <tr>
                                            <td class="whitespace-nowrap text-center">{{ $index + $imunisasis->firstItem() }}</td>
                                            
                                            <td class="whitespace-nowrap">
                                                <div class="whitespace-nowrap font-medium text-gray-800 dark:text-gray-100;">
                                                    {{ $imunisasi->penduduk->nama_lengkap ?? 'N/A' }}
                                                </div>
                                            </td>
                                            
                                            <td class="whitespace-nowrap">
                                                <span class="badge-imunisasi">
                                                    {{ $imunisasi->jenis_imunisasi }}
                                                </span>
                                            </td>
                                            
                                            <td class="whitespace-nowrap text-gray-600 dark:text-gray-300">
                                                {{ \Carbon\Carbon::parse($imunisasi->tanggal_imunisasi)->format('d/m/Y') }}
                                            </td>
                                            
                                            <td class="whitespace-nowrap text-center space-x-2">
                                                <a href="{{ route('imunisasi.show', $imunisasi) }}" class="action-link link-detail">
                                                    <i class="fas fa-eye"></i>Detail
                                                </a>

                                                <a href="{{ route('imunisasi.edit', $imunisasi) }}" class="action-link link-edit">
                                                    <i class="fas fa-edit"></i>Edit
                                                </a>

                                                <form action="{{ route('imunisasi.destroy', $imunisasi) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data imunisasi ini? Aksi ini tidak dapat dibatalkan.');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @else
                            <p class="text-center text-gray-500 dark:text-gray-400 py-8">Tidak ada data imunisasi yang tercatat.</p>
                        @endif
                        
                        <div class="pagination-container">
                             <div class="text-sm text-gray-700 dark:text-gray-400">
                                Showing {{ $imunisasis->firstItem() }} to {{ $imunisasis->lastItem() }} of {{ $imunisasis->total() }} results
                            </div>
