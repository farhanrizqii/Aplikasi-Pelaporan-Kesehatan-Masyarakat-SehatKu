            ...
            <div class="data-card">
                <div class="text-gray-900 dark:text-gray-100">
                    ...
                    <div class="action-buttons-container"> 
                        <a href="{{ route('ibuhamil.create') }}" class="btn-tambah">
                            <i class="fas fa-plus"></i>
                            <span>Tambah Catatan Ibu Hamil</span>
                        </a>
                        <a href="{{ route('ibuhamil.export') }}" class="btn-ekspor">
                    ...
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('ibuhamil.index') }}" class="flex items-center w-full md:w-1/3 mb-8">
                        <input type="text" name="search" placeholder="Cari Nama atau NIK Ibu..." 
                            value="{{ request('search') }}"
                            class="search-input"
                            aria-label="Cari data ibu hamil">
                        ...
                        @if (request('search'))
                            <a href="{{ route('ibuhamil.index') }}" class="text-sm text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 font-semibold ml-3">Reset</a>
                        @endif
                    </form>
                    ...
                    <div class="overflow-x-auto">
                            ...
                            <table class="data-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left">Usia Kehamilan (Mg)</th>
                                        <th class="text-left">TPL (Perkiraan Lahir)</th>
                                    ...
                                    @foreach ($ibuHamils as $index => $ih)
                                        <tr>
                                            ...
                                            <td class="whitespace-nowrap text-gray-600 dark:text-gray-300">
                                                {{ $ih->usia_kehamilan_minggu ?? '-' }}
                                            </td>
                                            
                                            <td class="whitespace-nowrap text-gray-600 dark:text-gray-300">
                                                {{ $ih->tanggal_perkiraan_lahir ? \Carbon\Carbon::parse($ih->tanggal_perkiraan_lahir)->format('d/m/Y') : '-' }}
                                            </td>
                                            
                                            <td class="whitespace-nowrap">
                                                <span class="badge-risiko 
                                                    {{ $ih->risiko_kehamilan == 'Rendah' ? 'badge-rendah' : '' }}
                                                    {{ $ih->risiko_kehamilan == 'Sedang' ? 'badge-sedang' : '' }}
                                                    {{ $ih->risiko_kehamilan == 'Tinggi' ? 'badge-tinggi' : '' }}">
                                                    {{ $ih->risiko_kehamilan }}
                                                </span>
                                            </td>
                                            
                                            <td class="whitespace-nowrap text-center space-x-2">
                                                <a href="{{ route('ibuhamil.show', $ih) }}" class="action-link link-detail">
                                                    <i class="fas fa-eye"></i>Detail
                                                </a>

                                                <a href="{{ route('ibuhamil.edit', $ih) }}" class="action-link link-edit">
                                                    <i class="fas fa-edit"></i>Edit
                                                </a>

                                                <form action="{{ route('ibuhamil.destroy', $ih) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus catatan ibu hamil ini? Aksi ini tidak dapat dibatalkan.');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @else
                            <p class="text-center text-gray-500 dark:text-gray-400 py-8">Tidak ada data ibu hamil yang tercatat.</p>
                        @endif
                        
                        <div class="pagination-container">
                             <div class="text-sm text-gray-700 dark:text-gray-400">
                                Showing {{ $ibuHamils->firstItem() }} to {{ $ibuHamils->lastItem() }} of {{ $ibuHamils->total() }} results
                            </div>
