            ...
            <div class="data-card">
                <div class="text-gray-900 dark:text-gray-100">
                    ...
                    <div class="action-buttons-container"> 
                        <a href="{{ route('laporan-kesehatan.create') }}" class="btn-tambah">
                            <i class="fas fa-plus"></i>
                            <span>Buat Laporan Baru</span>
                        </a>
                        <a href="{{ route('laporan-kesehatan.export') }}" class="btn-ekspor">
                    ...
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('laporan-kesehatan.index') }}" class="flex items-center w-full md:w-1/2 lg:w-1/3 mb-8">
                        <input type="text" name="search" placeholder="Cari Kegiatan, Fasilitas, atau Petugas..." 
                            value="{{ request('search') }}"
                            class="search-input"
                            aria-label="Cari laporan kesehatan">
                        ...
                        @if (request('search'))
                            <a href="{{ route('laporan-kesehatan.index') }}" class="text-sm text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 font-semibold ml-3">Reset</a>
                        @endif
                    </form>
                    ...
                    <div class="overflow-x-auto">
                            ...
                            <table class="data-table">
                                ...
                                <tbody>
                                    @foreach ($laporans as $index => $laporan)
                                        <tr>
                                            <td class="whitespace-nowrap text-center">{{ $index + $laporans->firstItem() }}</td>
                                            
                                            <td class="whitespace-nowrap font-medium text-gray-800 dark:text-gray-100">
                                            ...
                                            <td class="whitespace-nowrap text-center space-x-2">
                                                <a href="{{ route('laporan-kesehatan.show', $laporan) }}" class="action-link link-detail">
                                                    <i class="fas fa-eye"></i>Detail
                                                </a>

                                                <a href="{{ route('laporan-kesehatan.edit', $laporan) }}" class="action-link link-edit">
                                                    <i class="fas fa-edit"></i>Edit
                                                </a>

                                                <form action="{{ route('laporan-kesehatan.destroy', $laporan) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus laporan ini? Aksi ini tidak dapat dibatalkan.');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @else
                            <p class="text-center text-gray-500 dark:text-gray-400 py-8">Belum ada laporan kesehatan yang tercatat.</p>
                        @endif
                        
                        <!-- Pagination -->
                        <div class="pagination-container">
                            <div class="text-sm text-gray-700 dark:text-gray-400">
                                Showing {{ $laporans->firstItem() ?? 0 }} to {{ $laporans->lastItem() ?? 0 }} of {{ $laporans->total() }} results
                            </div>
